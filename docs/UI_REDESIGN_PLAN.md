...
B4B 当前实现：活动入口 `home/index.vue` 已移除静态订单、计划、访问和温压图表，仅只读展示现有 userInfo Store 的用户、组织、角色，以及明确的“业务数据尚未接入”状态；未实现快捷入口。`home/backup/index.vue` 保留原演示实现，不清理其潜在动态引用。此前两文件相同的描述属于改造前基线。不能把示例值换名“检测中”继续展示。

Personal Center 已按身份信息、个人资料、账户安全分区呈现并使用现有 Token；原 API、上传、密码修改、校验与提交脚本保持。页面展示验证不代表头像上传、资料保存或密码修改的写入 UAT 已通过。
