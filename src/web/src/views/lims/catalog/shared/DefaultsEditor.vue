<template>
  <div class="defaults-editor" v-loading="loading">
    ...
    <div v-for="field in fields" :key="field.key" class="default-field">
      <el-checkbox :model-value="has(field.key)" :disabled="disabled" @change="toggle(field, $event)"
        >{{ field.label }}（{{ field.key }}）</el-checkbox
      >
      <template v-if="has(field.key)">
        <el-checkbox
          ...
          :disabled="disabled"
          @change="set(field.key, $event ? null : initial(field.type))"
          >空值</el-checkbox
        >
        <ValueInput
          v-if="modelValue?.[field.key] !== null"
          :field="field"
...
import { api as productApi } from '../product/api';
...
const unknown = computed(() =>
  Object.keys(props.modelValue || {}).filter((key) => !fields.value.some((field) => field.key === key))
);
...
function toggle(field: any, enabled: any) {
  if (enabled) set(field.key, Object.hasOwn(field, 'default') ? field.default : initial(field.type));
  else remove(field.key);
}
let serial = 0;
watch(
  () => [props.serviceId, props.productId],
  async () => {
    const current = ++serial;
    fields.value = [];
    error.value = '';
    if (!props.serviceId && !props.productId) return;
    loading.value = true;
    try {
      let id = props.serviceId;
      if (props.productId) id = (await productApi.retrieve(props.productId)).data.service;
      if (!id) throw new Error('技术服务不可读');
      const res = await serviceApi.retrieve(id);
      if (!Array.isArray(res.data.requirement_template)) throw new Error('没有参数模板读取权限');
      if (current === serial)
        fields.value = res.data.requirement_template.filter(
          (field: any) => !['file', 'image', 'csv', 'table'].includes(field.type)
        );
    } catch {
      if (current === serial) error.value = '无法读取参数模板，请检查技术服务与产品读取权限。';
    } finally {
