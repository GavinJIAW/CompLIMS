...
export function createCrudOptions({ context }: any) {
  ...
  const actions = {
    sign: {
      text: '签署',
      show: compute(({ form }: any) => !!form?.id && form.status === 'DRAFT' && auth('contract:Sign')),
      click: async (scope: any) => {
        try {
          await ElMessageBox.confirm('确认签署？', '签署', { type: 'warning' });
          const result = await api.sign(scope.form.id);
    ...
    void: {
      text: '作废',
      show: compute(({ form }: any) => !!form?.id && form.status === 'SIGNED' && auth('contract:Void')),
      click: async (scope: any) => {
        try {
    ...
    crudOptions: {
      columns: {
        ...
        contract_date: {
          title: '合同日期',
          type: 'input',
          column: { show: !!permissions.contract_date?.is_query, minWidth: 150 },
          search: { show: !!permissions.contract_date?.is_query, component: { clearable: true } },
          form: {
            col: { span: 12 },
            component: {
              disabled: compute((scope: any) => disabled(scope, 'contract_date')),
              name: 'el-date-picker',
              valueFormat: 'YYYY-MM-DD',
            },
            rules: [{ required: true, message: '请填写合同日期' }],
          },
          addForm: { show: !!permissions.contract_date?.is_create },
          editForm: { show: !!permissions.contract_date?.is_update || !!permissions.contract_date?.is_query },
          viewForm: { show: !!permissions.contract_date?.is_query },
        },
        customer: {
          title: '关联客户',
          type: 'input',
          ...
          form: {
            ...
            render: (scope: any) =>
              h(SchemeEditor, {
                ...
                groupPermissions: permissions._children?.ContractScheme || {},
                itemPermissions: permissions._children?.ContractSchemeItem || {},
              }),
          },
        ...
        source_quotation: {
          title: '来源报价',
          type: 'input',
          column: { show: !!permissions.source_quotation?.is_query, minWidth: 150 },
          form: { show: !!permissions.source_quotation?.is_query, component: { disabled: true } },
          addForm: { show: false },
          search: { show: false },
        },
        subtotal: {
      ...
      rowHandle: {
        ...
        buttons: {
          ...
          remove: { show: compute(({ row }: any) => auth('contract:Delete') && row.status === 'DRAFT') },
        },
      },
      form: {
        wrapper: {
        ...
        group: {
          groupType: 'tabs',
          groups: {
            tab0: { label: '基本信息', columns: ['number', 'contract_date', 'status', 'source_quotation'] },
            tab1: {
              label: '客户与联系人',
              columns: [
                'customer',
