...
const prefix = '/api/lims/contract/';
export const api = {
  ...
  sign: (id: number, data: any = {}) => request({ url: `${prefix}${id}/sign/`, method: 'post', data }),
  ...
  if (result.schemes)
    result.schemes = result.schemes.map((row: any) => {
      const value = childPayload(
        ...
        permissions._children?.ContractScheme,
        ['source_scheme']
      );
      if (!row.id && row.source_scheme) delete value.items;
        ...
        value.items = value.items.map((item: any) =>
          childPayload(
            ...
            permissions._children?.ContractSchemeItem,
            ['source_product']
          )
        );
      return value;
