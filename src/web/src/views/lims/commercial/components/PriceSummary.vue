<template>
  <el-descriptions :column="3" border>
    <el-descriptions-item label="小计 / 人民币含税元">{{previewSubtotal ?? form.subtotal ?? '—'}}</el-descriptions-item>
    <el-descriptions-item label="调整 / 元">{{form.adjustment_amount ?? '0.00'}}</el-descriptions-item>
    <el-descriptions-item label="总额 / 人民币含税元">{{previewTotal ?? form.total_amount ?? '—'}}</el-descriptions-item>
  </el-descriptions>
</template>
...
const previewSubtotal=computed(()=>{
  ...
  return sum(rows.map((row:any)=>sum([multiply(row.quantity,row.unit_price)],2)),2);
});
const previewTotal=computed(()=>{
  ...
  const cents=(text:string)=>{const sign=text.startsWith('-')?-1n:1n;const [whole,fraction='']=text.replace('-','').split('.');return sign*(BigInt(whole)*100n+BigInt(fraction.padEnd(2,'0')));};
  const value=cents(subtotal)+cents(adjustment), digits=(value<0n?-value:value).toString().padStart(3,'0');
