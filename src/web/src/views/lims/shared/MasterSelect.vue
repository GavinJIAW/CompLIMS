<template>
  <el-select :model-value="modelValue" clearable :disabled="disabled" filterable remote :remote-method="search" :loading="loading" placeholder="搜索编号或名称" @visible-change="visible => visible && search('')" @update:model-value="select">
    <el-option v-for="row in options" :key="row.id" :value="row.id" :label="`${resource === 'cost_type' ? (row.name || row.id) : `${row.number || row.id} · ${row.name || ''}`}${row.enabled === false ? '（已停用）' : ''}`" :disabled="!includeDisabled && row.enabled === false && row.id !== modelValue" />
  </el-select>
</template>
<script setup lang="ts">
import { ref, watch } from 'vue';
import { request } from '/@/utils/service';
const props = defineProps<{ modelValue?: number; resource: string; disabled?: boolean; includeDisabled?: boolean }>();
const emit = defineEmits(['update:modelValue', 'selected']);
const options = ref<any[]>([]), loading = ref(false);
let serial = 0;
async function search(term: string) {
  const current = ++serial; loading.value = true;
  try { const res = await request({ url: `/api/lims/${props.resource}/`, params: { search: term, ...(props.includeDisabled ? {} : { enabled: true }), limit: 50 } });
