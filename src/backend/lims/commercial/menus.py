...
from .access_contract import READ, CHILD_READ, CHILDREN, ACTIONS


def initialize_commercial():
    roots = {}
    for index, (name, title) in enumerate([('commercial', '商务管理')]):
        root_path = '/lims/commercial'
        roots[name], _ = Menu.objects.update_or_create(component_name=f'lims_{name}_root', defaults={'name': title, 'web_path': root_path, 'is_catalog': True, 'sort': 23 + index, 'status': True})
    for resource, model, title in [('quotation', 'Quotation', '报价'), ('contract', 'Contract', '合同')]:
        group = 'commercial'
        ...
        if existing:
            MenuField.objects.filter(menu=existing, field_name='contact_phone_snapshot').delete()
