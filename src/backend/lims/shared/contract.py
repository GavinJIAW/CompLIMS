...
READ = {
    ...
    'cost_item': MASTER + ' cost_type unit_cost unit basis_data',
...
WRITE = {key: 'number name enabled description ' + fields for key, fields in {
    ...
    'cost_type': '',
    'cost_item': 'cost_type unit_cost unit basis_data',
    'cost_package': 'unit items',
    'product': NAMES + ' service requirement_defaults unit reference_price packages',
    'scheme': NAMES + ' items',
}.items()}
CLASSES = {'cost_type': 'CostTypeViewSet', 'service': 'ServiceViewSet', 'cost_item': 'CostItemViewSet',
...
MODULES = {resource: 'lims.' + ('costing' if resource in ('cost_type', 'cost_item', 'cost_package') else 'catalog') + '.views' for resource in CLASSES}
