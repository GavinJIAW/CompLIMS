...
class RoleMenuButtonPermissionViewSet(CustomModelViewSet):
    ...
    def get_role_menu(self, request):
        ...
        if is_superuser:
            menu_queryset = Menu.objects.filter(status=1).all()
        else:
            menu_id_list = request.user.role.values_list('role_menu__menu__id',flat=True)
            menu_queryset = Menu.objects.filter(id__in=menu_id_list,status=1).all()
