...
class MenuButton(CoreModel):
    ...
    method = models.IntegerField(default=0, verbose_name="接口请求方法", null=True, blank=True,
                                 help_text="接口请求方法")
    sort = models.IntegerField( verbose_name="排序", help_text="排序",default=0)
...
def media_file_name(instance, filename):
    ...
    if instance.file_url:
        file_folder = instance.file_url
        url = os.path.join(file_folder, h + ext.lower())
        instance.file_url = f'media/{url}'
        return url
    else:
        url = os.path.join("files", h[:1], h[1:2], h + ext.lower())
        instance.file_url = f'media/{url}'
        return url
